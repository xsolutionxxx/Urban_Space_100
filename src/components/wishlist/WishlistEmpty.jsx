...
function WishlistEmpty({ ctaText = "Переглянути популярні", ctaTo = "/" }) {
  return (
    <div className="flex-1 flex items-center justify-center flex-col gap-6">
      ...
      <Link
        to={ctaTo}
        className="relative flex justify-center gap-1 font-medium text-sm uppercase border-b"
        aria-label="Перейти до каталогу товарів"
      >
        {ctaText}
        <ExternalLink strokeWidth={2} size={12} className="absolute top-0.5 -right-4" />
      </Link>
    </div>
  );
}
